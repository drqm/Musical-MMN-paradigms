...
if isfield(cfg,'mel_pool') && ~isempty(cfg.mel_pool)
    ...
    all_melodies = {[1 8 5 8 1 8 5 8 1 8 5 8 1 8 5 8 1 8 5 8 1 8 5 8 1 8 5 8 1 8 5 8]};
end
...
if isfield(cfg,'baseline_add') && ~isempty(cfg.baseline_add)
    ...
    baseline_add = 1; % add one melody without deviants at the beggining of the block
end
...
if isfield(cfg,'n_deviants') && ~isempty(cfg.n_deviants)
    ...
    n_deviants = 4; % Number of standard tones in the tone pool
end

if isfield(cfg,'cell_size') && ~isempty(cfg.cell_size)
    cell_size = cfg.cell_size;
else
    cell_size = 4; % Number of standard tones in the tone pool
end
...
if n_runs > 1
    for run = 2:n_runs
        mel_prob = 1;
        while mel_prob == 1 % will only run as long as trans_prob = 1
            old_run = random_mel;
